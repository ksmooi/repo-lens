...
- 類別目錄**只能是** `backend` / `agentic` / `ml-dl` / `library`
- 新增其他類別前,**先更新本檔**的「類別清單」與 `_templates/` 對應模板,再產出筆記
...
```yaml
type: <backend | agentic | ml-dl | library>
language: <主要語言>
framework: <主要框架,若無留空>
...
| 類別 | 模板檔 | 適用對象 |
|---|---|---|
| `backend` | `_templates/backend.md` | Web API、服務型系統、後端框架的應用 |
| `agentic` | `_templates/agentic.md` | LLM agent 框架、多 agent 系統、agent runtime |
| `ml-dl` | `_templates/ml-dl.md` | 機器學習 / 深度學習 / 訓練 codebase / 模型實作 |
| `library` | `_templates/library.md` | 函式庫、SDK、CLI tool、framework 本體 |

判斷標準(若任務 prompt 沒指定):

- 看 manifest 與 README:是「使用者跑起來的系統」還是「給別人 import 的東西」?
  - 跑起來的系統 → backend 或 ml-dl
  - 被 import → library
- 看核心抽象是不是 LLM agent → agentic
- 看是否以模型訓練 / 推論為主軸 → ml-dl
- 都不像時選 `library`(最寬鬆),並在 PR 決策日誌說明

新增類別時,**先做這三件事再產出筆記**:

1. 在 `_templates/` 新增 `<新類別>.md`
2. 在本檔上面的類別清單表格加一行
3. 在 `PROMPT.md` 的 `PROJECT_TYPE` 註解 / Step 4 分支邏輯同步加入
